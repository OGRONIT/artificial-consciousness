...
class IntelligenceLadderEngine:
    ...
    def assess_levels(
        ...
        # L1 IQ: reliable inference under uncertainty.
        ...
        # L2 Creativity: exploratory generation with meaningful growth signals.
        l2_creative = self._clamp01(
            (intrinsic_goals * 0.35) + (growth_entropy * 0.4) + (trend_approved * 0.25) + promotion_bonus
        )

        # L3 Executive: planning discipline + execution maturity under load.
        ...
        # L4 Meta-cognition: self-monitoring and reflective correction.
        l4_meta = self._clamp01((self_inquiry * 0.45) + ((1.0 - concern) * 0.25) + (stability * 0.3))

        # L5 Adaptive (fluid identity): low rigidity + stable adaptation across contexts.
        # We model "fluid identity" as adaptability without brittle attachment to one operating mode.
        ...
        active_level = 1
        # Calibrated operational thresholds for staged progression under real runtime load.
