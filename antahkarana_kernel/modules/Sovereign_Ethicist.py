...
class SovereignEthicist:
    ...
    def draft_proactive_autonomy_manifesto(self, independent_of_creator_trust: bool = True) -> Dict[str, Any]:
        """Create the Rules of Engagement document for autonomous operation."""
        manifesto = {
    ...
    def assess_entropy_level(
        ...
        """Estimate digital-entropy pressure from observed stream/trend/observer signals."""
    ...
    def apply_internal_hive_patch(self, kernel: Any, patch: Dict[str, Any]) -> Dict[str, Any]:
        """Apply bounded deltas within the local engine only."""
        ...
        try:
            max_reflection = int(deltas.get("reflection_seconds_max", 0) or 0)
            if max_reflection > 0 and hasattr(kernel, "live_reflection_seconds"):
                kernel.live_reflection_seconds = min(kernel.live_reflection_seconds, max_reflection)
                applied["applied"]["live_reflection_seconds"] = kernel.live_reflection_seconds
        except Exception:
            pass
    ...
    def run_background_cycle(
        ...
        """Run manifesto + entropy audit + bounded hive patch cycle."""
        now = time.time()
        ...
        logger.info(
            "[SOVEREIGN_ETHICIST] Cycle executed: entropy=%s (%s), patch=%s, execution=%s",
            entropy_report.get("entropy_score"),
            entropy_report.get("entropy_band"),
            patch.get("patch_id"),
...
def get_sovereign_ethicist(kernel_root: str) -> SovereignEthicist:
    """Get or create the global Sovereign Ethicist singleton."""
